              ...
              <div class="person-list-h-535 overflow-y-auto">
                <ul>
                    ...
                    @foreach ($buyers as $index => $buyer)
                        <li class="d-flex justify-content-between align-items-center p-3 message-person selerItem" data-buyer_id="{{ $buyer->buyer_id }}">
                            <div class="d-flex align-items-center gap-3">
                                <div class="image-unread-feature position-relative">
                                    @if ($buyer?->buyer?->image)
                                    <img
                                    src="{{ asset($buyer?->buyer?->image) }}"
                                    ...
                                    @php
                                        ...
                                        $unseen_message = Modules\LiveChat\App\Models\Message::where('seller_id', $seller->id)->where('buyer_id', $buyer->buyer_id)->where('send_by', 'buyer')->where('seller_read_msg', 0)->count();
                                    @endphp

                                    <div class="unread-message-qty {{ $unseen_message == 0 ? 'd-none' : '' }} unread-message-{{ $buyer->buyer_id }}">
                                        <span>{{ $unseen_message }}</span>
                                    </div>
                               </div>

                                <div>
                                <h4 class="text-18 fw-normal">{{ html_decode($buyer?->buyer?->name) }}</h4>
                                <p class="text-dark-200 fs-6">
                                    {{ html_decode($buyer?->buyer?->designation) }}
                                </p>
                                </div>
                            </div>
                            @php
                                ...
                                $last_message = Modules\LiveChat\App\Models\Message::where('seller_id', $seller->id)->where('buyer_id', $buyer->buyer_id)->where('send_by', 'buyer')->latest()->first();
                            @endphp
                            @if ($last_message)
                                <div>
        ...
        (function($) {
            ...
            $(document).ready(function () {
                $(".selerItem").on("click", function(e){
                    ...
                    $(`.unread-message-${buyer_id}`).addClass('d-none');
                    $.ajax({
                        type:"get",
                        url:"{{url('/seller/get-message-body/') }}"+"/"+buyer_id,

<div class="d-flex justify-content-between border-bottom py-3 px-4" >
    ...
    <div class="d-md-block d-none">
        <a href="{{ route('freelancer', $seller->username) }}" target="_blank" class="w-btn-primary-lg">{{ __('translate.View Profile') }}</a>
    </div>
</div>
    ...
    <script>
        ...
        function loadedLatestMessage(){
            setInterval(() => {
                ...
                $.ajax({
                    type:"get",
                    url:"{{url('/buyer/get-message-list/') }}"+"/"+seller_id,
                    success:function(response){
                        $(".message-list").html(response);
                        scrollToBottomFunc();
                    },
